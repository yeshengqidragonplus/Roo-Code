...
| 7                   | **工作流可视化编辑器** | 引擎已就绪，但无 UI 编辑器（用户无法创建工作流）                                   | React Flow 编辑器集成 webview      | 🔴 大 | P1     |
...
- [ ] 集成 React Flow（`@xyflow/react`）到 webview-ui
- [ ] 节点类型渲染：llm / condition / tool / skill / expert / parallel
- [ ] 节点配置面板：prompt 编辑、outputSchema、exec 软/硬、参数引用 `{{node.output}}`
- [ ] 边配置：condition 的 `branch: true/false`
- [ ] 图 JSON 导入/导出（与 AIWorkflow 引擎格式对齐）
- [ ] 保存到 `.roo/workflows/`
- [ ] 在 mode 创建界面绑定工作流
- [ ] 这是让用户**能自己创建工作流**的关键，否则工作流功能对普通用户不可用
