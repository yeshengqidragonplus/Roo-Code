...
describe("MockWorkspaceConfiguration", () => {
	...
	describe("Runtime Configuration", () => {
		...
		it("should return runtime config value over disk-based values", async () => {
			const config = new MockWorkspaceConfiguration("qcode", context)
			...
			setRuntimeConfig("qcode", "commandExecutionTimeout", 20)
		...
		it("should set and get runtime config values", () => {
			setRuntimeConfig("qcode", "testSetting", "testValue")

			expect(getRuntimeConfig("qcode.testSetting")).toBe("testValue")
		})

		it("should set multiple runtime config values at once", () => {
			setRuntimeConfigValues("qcode", {
				setting1: "value1",
				setting2: 42,
				setting3: true,
			})

			expect(getRuntimeConfig("qcode.setting1")).toBe("value1")
			expect(getRuntimeConfig("qcode.setting2")).toBe(42)
			expect(getRuntimeConfig("qcode.setting3")).toBe(true)
		})
		...
		it("should ignore undefined values in setRuntimeConfigValues", () => {
			...
			expect(getRuntimeConfig("qcode.defined")).toBe("value")
			expect(getRuntimeConfig("qcode.notDefined")).toBeUndefined()
		})

		it("should clear all runtime config values", () => {
			setRuntimeConfig("qcode", "setting1", "value1")
			setRuntimeConfig("qcode", "setting2", "value2")

			clearRuntimeConfig()

			expect(getRuntimeConfig("qcode.setting1")).toBeUndefined()
			expect(getRuntimeConfig("qcode.setting2")).toBeUndefined()
		})
