...
const extension = vscode.extensions.getExtension<RooCodeAPI>("QCode.qcode")
...
**NOTE:** To ensure that the `QCode.qcode` extension is activated before your extension, add it to the `extensionDependencies` in your `package.json`:

```json
"extensionDependencies": ["QCode.qcode"]
```
